<template>
  ...
  <button
    ...
    class='align-items-center flex gap-2 p-link topbar-profile-button' type='button'>
    ...
    <li>
      <a v-ripple
        ...
        <span class='align-items-center flex justify-content-between w-full'>{{ t('common.language') }}
          <Dropdown class='line-height-1' :model-value='language' :options='["en", "vi"]'
      ...
      <a v-ripple
        ...
        <span class='align-items-center flex justify-content-between w-full' @click='changeTheme'>{{ t('common.theme')
          }}
          <i class='p-2' :class='dark ? "pi pi-moon text-blue-700" : "pi pi-sun text-yellow-700"'></i>
        </span>
      </a>

      <router-link v-ripple
        class='align-items-center border-round cursor-pointer flex h-3rem hover:surface-hover p-2 p-ripple transition-colors transition-duration-150'
        style='color: var(--text-color)' :to='{ name: "Profile" }'>
