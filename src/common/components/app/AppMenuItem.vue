<template>
  <li
    ...
    <div
      ...
      <span v-else>{{ item.label }}</span> <i class='layout-menuitem-root-icon'></i>
    </div>
    <a
      v-if='(!item.to || item.items) && item.visible !== false'
      :class='(hasActiveChild(item) ? "active-menu " : "") + (item.class ?? "")'
      :href='item.url'
      tabindex='0'
      :target='item.target'
      ...
      <i
        v-if='item.items'
        class='layout-submenu-toggler pi pi-angle-down pi-fw'></i>
    </a>
    <router-link
      v-if='item.to && !item.items && item.visible !== false'
...
watch(
  ...
  () => {
    ...
    const overlay = target.querySelector('.layout-root-submenulist');
...
function hasActiveChild (item) {
  if (item.items) {
    return item.items.some((child) => {
      return checkActiveRoute(child) || hasActiveChild(child);
    });
  }
