...
export function useLayout() {
  ...
  const changeThemeAndColorScheme = (theme: string, colorScheme: string) => {
    const themeLink = document.getElementById('theme-link');
    const themeLinkHref = themeLink?.getAttribute('href');
    const currentColorScheme = 'theme-' + layoutConfig.colorScheme.toString();
    const newColorScheme = 'theme-' + colorScheme;
    const newHref = themeLinkHref?.replace(currentColorScheme, newColorScheme).replace(layoutConfig.theme, theme);
  ...
  const replaceLink = (linkElement: HTMLElement | null, href: string | undefined, onComplete: () => void) => {
    if (!linkElement || !href) {
      return;
    }
