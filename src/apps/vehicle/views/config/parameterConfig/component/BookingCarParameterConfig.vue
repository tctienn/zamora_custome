<template>
  <div
    ...
    <Accordion
      ...
      :multiple='true'>
      <AccordionTab header='1. Thiết lập bộ mã Phiếu đặt xe tự sinh'>
        <div
...
onResult((res) => {
  parameter.value = cloneDeep(res.data.findParameterConfig.value.bookingCar);
});
const {
  mutate,
  onDone
...
function save() {
  ...
  mutate({ parameterConfig: { value: { bookingCar: parameter.value } } });
}

onDone(() => {
  toastSuccess({ message: 'Lưu thiết lập bộ mã Phiếu đặt xe thành công' });
});
