<template>
    ...
    <template #start>
      <div class='align-items-center flex gap-2'>
        <div class='font-bold ml-3'>
          {{ t(`media.common.list`) }} {{ t(`book-ticket.menu.bookTicketCountry`) }}
        </div>
      </div>
    </template>
    <template #end>
  ...
  <DataTable
    ...
    <Column
      class='col-1'
      field='code'
      :header='t(`book-ticket.country.code`)'></Column>
    <Column
      class='col-1 pl-0'
      field='name'
      :header='t(`book-ticket.country.name`)'></Column>
    <Column
      class='col-2'
      field='listCity'
      :header='t(`book-ticket.country.city.name`)'>
      <template #body='{ data: { listCity } }'>
        <div
          v-if='Array.isArray(listCity)'
          class='break-words whitespace-normal'>
          {{ listCity.map(city => city.cityName).join('; ') }}
        </div>
      </template>
    </Column>
    <Column
...
import { deleteCountry, searchCountry } from '../../api/graphql/country';
import type { ICountry } from '../../model/country';
...
const selectedRow = ref<ICountry>();
...
const {
  onResult: getCountryResult,
  refetch: getCountryRefetch,
  onError: getCountryError,
} =
  searchCountry(searchParams);

watch(searchTerm, (newValue) => {
  searchParams.searchInput.textSearch = newValue;
  isLoading.value = true;
  getCountryRefetch(searchParams);
});

const {
  mutate: deleteCountryMutate,
  onDone: deleteCountryOnDone,
  onError: deleteCountryOnError,
} =
  deleteCountry();
...
getCountryResult((res) => {
  const {
    totalRecords,
    content
  } = get(res, 'data.searchCountry', {
    totalRecords: 0,
    content: [],
  });
...
function toggleActionMenu(event: Event, data: ICountry) {
...
const handleDeleteVehicle = () => {
  deleteCountryMutate({ id: selectedRow.value?.id });
};
...
deleteCountryOnError(() => {
  toastError({ message: t('book-ticket.country.deleteInUseError') });
});
</script>
