...
import { deleteDocumentGraph, searchDocumentFrom } from '@/apps/ecm/api/graphql/document-from-graphql';
import type { DocumentFromEdge } from '@/apps/ecm/model/document';
...
const documents = ref([]);
const document = ref({} as DocumentFromInterface);
...
const searchParams = reactive<{ keyword: string, pageable: Pageable }>(
  query.searchParams ? JSON.parse(query.searchParams as string)
    : {
      keyword: '',
      pageable: pageable
    }
...
const {
  onResult: searchDocumentFromOnResult,
  refetch: searchDocumentRefetch
} = searchDocumentFrom(searchParams);
searchDocumentFromOnResult((result) => {
  const {
    totalCount,
    edges
  } = get(result, 'data.searchDocumentFrom', {
    totalCount: 0,
    edges: []
  });
  totalRecords.value = totalCount;
  documents.value = edges.map((edge: DocumentFromEdge, index: number) => ({
    ...edge.node,
...
function toggleActionMenu({
  event,
  data
}: { event: Event, data: DocumentFromInterface }) {
...
<script lang='ts'>
export default { name: 'DocumentFromManage' };
</script>
