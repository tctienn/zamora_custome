    ...
    <div class='card'>
      ...
      <div class='p-fluid'>
        <div class='formgrid grid'>
          ...
          <!--  Chỗ khác duy nhất với Tạo văn bản đến: Đơn vị lấy số đăng ký -->
          <div class='col-3 field'>
            <label
              v-required
              for='organizationCode'>{{ t('ecm.document.unitGetRegistrationNumber') }}</label>
            <NamedDropdown
              v-model='document.unitGetRegistrationNumber'
              :filter='true'
              name='unitGetRegistrationNumber'
              option-label='name'
              option-value='code'
              :options='unitGetRegistrationNumbers'/>
            <ValidateErrorMessage :errors='v$.unitGetRegistrationNumber.$errors'/>
          </div>
...
import { findDocumentToById } from '@/apps/ecm/api/graphql/document-to-graphql';
import { createOrUpdateDocumentTo } from '@/apps/ecm/api/rest/document';
...
const unitGetRegistrationNumbers = ref([]);
...
if (id) {
  const { onResult: findDocumentByIdOnResult } = findDocumentToById(id as string);
  fileAttachInvalid.value = false;
  findDocumentByIdOnResult((res) => {
    document.value = { ...res.data.findDocumentToById };
...
let document = ref<DocumentToInterface>({} as DocumentToInterface);
...
const { onResult: getUnitGetRegistrationNumbersOnResult } = getCategoryByGroup('UNIT');
getUnitGetRegistrationNumbersOnResult(res => {
  unitGetRegistrationNumbers.value = res.data.getCategoryByGroup;
});
...
const validateRules = {
  ...
  unitGetRegistrationNumber: { required: helpers.withMessage(t('ecm.document.message.unitGetRegistrationNumberEmpTy'), required) },
...
function submit() {
  ...
  v$.value.$validate().then((isValid) => {
    if (isValid && !fileAttachInvalid.value) {
      ...
      createOrUpdateDocumentTo(formData).then(() => {
        router.replace({
          name: 'EcmDocumentTo',
...
<script lang='ts'>
export default { name: 'FormDocumentTo' };
</script>
