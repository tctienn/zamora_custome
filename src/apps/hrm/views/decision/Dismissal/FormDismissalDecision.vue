        ...
        <form ref='formRef'>
          <Accordion
            ...
            <AccordionTab :header='t("hrm.contract.general.title")'>
              <DecisionDismissalForm v-model:model-value='modelValue'/>
            </AccordionTab>
            <AccordionTab
...
import DecisionDismissalForm from '@/apps/hrm/views/decision/Dismissal/DecisionDismissalGeneralForm.vue';
...
async function submit() {
  ...
  if (formRef.value) {
    ...
    if (!modelValue.value.id) {
      createDismissalDecision(data).then(() => {
        toastSuccess({ message: 'Thêm mới quyết định thành công.' });
        emits('hide-dialog');
      });
    } else {
      updateDismissalDecision(modelValue.value.id, data).then(() => {
