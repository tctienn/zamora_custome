        ...
        <form ref='formRef'>
          <Accordion
            ...
            <AccordionTab :header='t("hrm.contract.general.title")'>
              <DecisionTerminationGeneralForm v-model:model-value='modelValue'/>
            </AccordionTab>
...
import DecisionGeneralForm from '@/apps/hrm/views/decision/AppointmentDecision/DecisionGeneralForm.vue';
import DecisionTerminationGeneralForm
  from '@/apps/hrm/views/decision/TerminationDecision/DecisionTerminationGeneralForm.vue';
...
async function submit() {
  ...
  if (formRef.value) {
    ...
    if (!modelValue.value.id) {
      createTerminationDecision(data).then(() => {
        toastSuccess({ message: 'Thêm mới quyết định thành công.' });
        emits('hide-dialog');
      });
    } else {
      updateTerminationDecision(modelValue.value.id, data).then(() => {
