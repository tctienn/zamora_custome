        ...
        <form ref='formRef'>
          <Accordion
            ...
            <AccordionTab :header='t("hrm.contract.general.title")'>
              <DecisionGeneralForm v-model:model-value='modelValue'/>
            </AccordionTab>
            <AccordionTab
...
import { createDecision, updateDecision } from '@/apps/hrm/api/rest/decision';
...
async function submit() {
  ...
  if (formRef.value) {
    ...
    if (!modelValue.value.id) {
      createDecision(data).then(() => {
        toastSuccess({ message: 'Thêm mới quyết định thành công.' });
        emits('hide-dialog');
      });
    } else {
      updateDecision(modelValue.value.id, data).then(() => {
