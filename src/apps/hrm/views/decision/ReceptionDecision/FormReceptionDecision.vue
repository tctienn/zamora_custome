        ...
        <form ref='formRef'>
          <Accordion
            ...
            <AccordionTab :header='t("hrm.contract.general.title")'>
              <DecisionReceptionGeneralForm v-model:model-value='modelValue'/>
            </AccordionTab>
...
import DecisionReceptionGeneralForm from '@/apps/hrm/views/decision/ReceptionDecision/DecisionReceptionGeneralForm.vue';
...
async function submit() {
  ...
  if (formRef.value) {
    ...
    if (!modelValue.value.id) {
      createReceptionDecision(data).then(() => {
        toastSuccess({ message: 'Thêm mới quyết định thành công.' });
        emits('hide-dialog');
      });
    } else {
      updateReceptionDecision(modelValue.value.id, data).then(() => {
