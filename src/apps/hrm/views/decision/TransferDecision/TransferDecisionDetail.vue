<template>
  <div class='body'>
    <p class='font-semibold text-blue-800'>Thông tin chung</p>
    <div class='formgrid grid px-2'>
      <div class='col-6 flex gap-2'>
        <p class='inline-block'>Số quyết định:</p>
        <p class='flex-1 font-semibold'>{{ decision?.noDecision }}</p>
      </div>

      <div class='col-6 flex gap-2'>
        <AppUser
          avatar-size='2'
          :user-id='decision?.createdBy'/>
        <div>
          <div class='text-sm'>
            {{
              t('document.doc.docInDetail.createBy')
            }}<b>{{ user?.fullName }}</b>
          </div>
          <div class='text-sm'>
            {{ t('document.doc.docInDetail.time') }}
            {{
              decision?.createdTime
                ? moment(decision?.createdTime).format('HH:mm DD/MM/YYYY')
                : ''
            }}
          </div>
        </div>

      </div>

      <div class='col-6 flex gap-2'>
        <p class='inline-block'>Ngày quyết định:</p>
        <p class='flex-1 font-semibold'>{{
          decision?.decisionDate
            ? moment(decision?.decisionDate).format('DD/MM/YYYY')
            : ''
        }}</p>
      </div>

      <div class='col-6 flex gap-2'>
        <p class='inline-block'>Cấp ra quyết định:</p>
        <p class='flex-1 font-semibold'>{{ decision?.decisionMakingLevelName }}</p>
      </div>

      <div class='col-6 flex gap-2'>
        <p class='inline-block'>Ngày hiệu lực:</p>
        <p class='flex-1 font-semibold'>{{
          decision?.effectStartDate
            ? moment(decision?.effectStartDate).format('DD/MM/YYYY')
            : ''
        }}</p>
      </div>

      <div class='col-6 flex gap-2'>
        <p class='inline-block'>Lý do điều chuyển:</p>
        <p class='flex-1 font-semibold'>{{ reasonName(decision?.reason) }}</p>
      </div>

      <div class='col-6 flex gap-2'>
        <p class='inline-block'>Nơi lập:</p>
        <p class='flex-1 font-semibold'>{{ decision?.placeOfDecision }}</p>
      </div>

      <div class='col-6 flex gap-2'>
        <p class='inline-block'>Người ký:</p>
        <p
          v-if='signer'
          class='flex-1 font-semibold'>{{ signer.fullName }}
          {{ signer.positionName ? `| ${signer.positionName}` : '' }}</p>
      </div>

      <table class='custom-table'>
        <thead>
          <tr>
            <th class='w-4rem'>STT</th>
            <th>Nhân sự</th>
            <th>Phòng ban cũ</th>
            <th>Chức danh/Vị trí cũ</th>
            <th>Chức vụ cũ</th>
            <th>Phòng ban bổ nhiệm</th>
            <th>Chức danh/Vị trí bổ nhiệm</th>
            <th>Chức vụ bổ nhiệm</th>
          </tr>
        </thead>
        <tbody>
          <tr
            v-for='(employee, index) in decision?.employeeDecisions'
            :key='index'
            class='font-semibold'>
            <td>{{ index + 1 }}</td>
            <td>{{ employee.employeeName }}</td>
            <td>{{ employee.oldDeptName }}</td>
            <td>{{ employee.oldTitleName }}</td>
            <td>{{ employee.oldPositionName }}</td>
            <td>{{ employee.newDeptName }}</td>
            <td>{{ employee.newTitleName }}</td>
            <td>{{ employee.newPositionName }}</td>
          </tr>
        </tbody>
      </table>

      <div class='col-12 flex gap-2 pt-2'>
        <p class='inline-block'>Mô tả:</p>
        <p class='flex-1 font-semibold'>{{ decision?.description }}</p>
      </div>
    </div>

    <p class='font-semibold text-blue-800'>Thông tin lương và phụ cấp</p>
    <div class='formgrid grid px-2'>
      <div class='col-6 flex gap-2'>
        <p class='inline-block'>Hình thức lương:</p>
        <p class='flex-1 font-semibold'>{{ getSalaryTypeLabel }}</p>
      </div>

      <div class='col-6 flex gap-2'>
        <p class='inline-block'>Hệ số lương:</p>
        <p class='flex-1 font-semibold'>{{ decision?.salaryDecision?.coefficient }}</p>
      </div>

      <div class='col-6 flex gap-2'>
        <p class='inline-block'>Ngạch lương:</p>
        <p class='flex-1 font-semibold'>{{ getScales }}</p>
      </div>

      <div class='col-6 flex gap-2'>
        <p class='inline-block'>Mức lương:</p>
        <p class='flex-1 font-semibold'>{{ decision?.salaryDecision?.coefficientSalary }}</p>
      </div>

      <div class='col-6 flex gap-2'>
        <p class='inline-block'>Bậc lương:</p>
        <p class='flex-1 font-semibold'>{{ decision?.salaryDecision?.grade }}</p>
      </div>

      <div class='col-6 flex gap-2'>
        <p class='inline-block'>Lương tối thiểu vùng:</p>
        <p class='flex-1 font-semibold'>{{ formatCurrencyWithDigits(decision?.salaryDecision?.regionalMinimum) }}</p>
      </div>

      <div class='col-12 flex gap-2'>
        <p class='inline-block'>Tổng lương:</p>
        <p class='flex-1 font-semibold'>{{ formatCurrencyWithDigits(decision?.salaryDecision?.totalSalary) }}</p>
      </div>

      <div class='col-12 flex gap-2'>
        <p class='font-semibold inline-block'>Các khoản phụ cấp</p>
      </div>

      <div class='pt-3'>
        <div
          v-for='(bonus,index) in decision?.salaryDecision?.bonuses'
          :key='index'
          class='col-12'>
          <div class='formgrid grid'>
            <div class='col-12 flex gap-1'>
              <p class='inline-block'>{{ bonus.type }}:</p>
              <p class='flex-1 font-semibold'> {{ formatCurrencyWithDigits(bonus.amount) }}</p>
            </div>
          </div>
        </div>
      </div>

    </div>

    <div class='flex font-semibold text-blue-800'>
      <AppIcon
        v-tooltip='t("chat.conversation.attachment")'
        name='attachment'
        size='1.5'/>
      <p> Tệp đính kèm</p>
    </div>

    <div
      v-for='(file, index) in decision?.files'
      :key='index'
      class='border-round file-row flex gap-2 p-2'>
      <div
        class='cursor-pointer flex gap-2 p-2 sm:hover:surface-ground w-full'
        @click="viewDetail(file.path ?? '')">
        <img
          alt='extension'
          :src='`/images/file-extension-icons/${getFileExtension(file.name ?? "")}.svg`'
          style='width:1.75rem'>
        <div class='flex flex-column flex-grow-1'>
          <div>{{ file.name }}</div>
          <div>{{ convertFileSize(file?.size ?? 0) }}</div>
        </div>
      </div>
    </div>
  </div>
</template>

<script lang='ts' setup>
import moment from 'moment';
import { storeToRefs } from 'pinia';
import { computed, onMounted, type PropType, ref } from 'vue';
import { useI18n } from 'vue-i18n';

import type { UserMoreInfo } from '@/apps/admin/model/User';
import { useUserMoreInfoStore } from '@/apps/admin/store/usersMoreInfo';
import { getAllByReasonType } from '@/apps/hrm/api/graphql/category';
import { CATEGORY_TYPE } from '@/apps/hrm/constants/category';
import { fetchCategoryMappingName } from '@/apps/hrm/helpers/utils';
import type { DecisionInterface } from '@/apps/hrm/model/decision';
import { GATEWAY_URL } from '@/common/api/configService';
import AppIcon from '@/common/components/app/AppIcon.vue';
import AppUser from '@/common/components/app/AppUser.vue';
import { convertFileSize } from '@/common/helpers/file-utils';
import { formatCurrencyWithDigits } from '@/common/helpers/utils';

const props = defineProps({
  decision: {
    type: Object as PropType<DecisionInterface>,
    default: {} as DecisionInterface
  }
});

const reasonName = computed(() => {
  return (code: string) => reasons.value?.find((r: any) => r.code == code)?.name;
});
const { t } = useI18n();
const store = useUserMoreInfoStore();
const { usersMoreInfo } = storeToRefs(store);
const { onResult: reasonResult } = getAllByReasonType('REASON_TRANSFER');
const reasons = ref();
reasonResult((res) => reasons.value = res.data.getAllByReasonType);
const scales = ref([]);
const user = computed(
  (): UserMoreInfo => usersMoreInfo.value[props.decision.createdBy],
);

const signer = computed((): UserMoreInfo => usersMoreInfo.value[props.decision.signerId]);
const getSalaryTypeLabel = computed(() => {
  const type = props.decision?.salaryDecision?.type;
  return t(`hrm.contract.salary.typeOptions.${type}`);
});

const getScales = computed(() => {
  const scale = props.decision?.salaryDecision?.scale ?? '';
  return scales.value.filter((item: string) => item.includes(scale))[0];
});

function getFileExtension(fileName: string) {
  return fileName.split('.').pop();
}

function viewDetail(filePath: string) {
  window.open(
    GATEWAY_URL
      + '/files/preview/'
      + filePath,
  );
}

function changeFileExtension(filename: string, newExtension: string): string {
  return filename.replace(/\.[^/.]+$/, `.${newExtension}`);
}

onMounted(() => {
  fetchCategoryMappingName(CATEGORY_TYPE.SALARY_STEP, scales);
});
</script>

<style scoped>
.custom-table {
  border-collapse: collapse;
  width: 100%;
  table-layout: auto;
  font-size: 13px;
}

td,
th {
  padding: 12px 8px;
  border: 1px solid #ccc;
  text-align: center;
  vertical-align: middle;
  word-break: break-word;
  white-space: normal;
  font-size: 13px;
}

.file-row {
  &:hover {
    .btnAction {
      display: block;
    }

    .file-name {
      color: #0F6CBDD6
    }
  }
}

.btnAction {
  display: none;
}

</style>