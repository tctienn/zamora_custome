      ...
      <div class='align-items-center flex justify-content-between w-full'>
        <div class='align-items-center flex gap-2'>
          ...
          <div class='p-sidebar-header-content'>
            {{ t(`hrm.interview.${interview?.id ? 'titleUpdate' : 'titleCreate'}`) }}
          </div>
        </div>
...
import HrmInterviewForm from '../form/interview/HrmInterviewForm.vue';

const { t } = useI18n();
const visible = defineModel('visible', { type: Boolean });
const interview = defineModel('interview', { type: Object as PropType<Interview> });
const v$: Ref<Validation> = useVuelidate();
const formRef = ref<InstanceType<typeof HrmInterviewForm>>();
