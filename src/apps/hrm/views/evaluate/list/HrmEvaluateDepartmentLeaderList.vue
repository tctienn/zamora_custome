<template>
  <DataTable
    ...
    <Column
      v-for='col in columnDepartmentLeadConfig'
      :key='col.key'
...
import { type ColumnConfig, columnDepartmentLeadConfig, generateHeaderDepartmentLeadConfig } from '@/apps/hrm/constants/generateHeaderConfig';
import {
  toastError,
  toastSuccess
...
const headerConfig = generateHeaderDepartmentLeadConfig(t, prevMonth.value, currentMonth.value);
...
function computeExtraScores() {
  localEmployees.value.forEach((emp: any) => {
    const avg = Number(emp.score.averageScoreOfMonthByWeek) || 0;
    const banTCNS = Number(emp.score.scoreOfBanTCNS) || 0;
    const total = avg + banTCNS;
    emp.score.scoreOfMonthByTruongBanAndBanTCNS = total ? Math.min(total, 10) : null;
    emp.score.roundedScoreOfMonth = total ? roundCustom(Math.min(total, 10)) : null;
  });
}
