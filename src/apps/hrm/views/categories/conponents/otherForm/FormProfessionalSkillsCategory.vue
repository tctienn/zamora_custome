...
import { cloneDeep, get, isNull, omit } from 'lodash';
...
import {
  ...
  getMaxOrderNumber,
  updateCategory,
} from '@/apps/hrm/api/graphql/category';
...
if (props.code) {
  getCategory(props.selectedCategory, props.code).onResult((res) => {
    ...
    parentCode.value = { [category.value.parentCode]: true };
  });
} else {
  setDefaultForm();
}
        ...
        if (isCreate) {
          createCategoryMutate({
            ...
            payload: {
              ...omit({
                ...category.value,
                parentCode: parentCode.value && Object.keys(parentCode.value)[0],
              }),
            },
          });
        } else {
...
const fetchProfessionalSkills = () => {
  const { onResult: getAllCategoryResult } = getAllCategory(
    userDeptRole.value,
    props.selectedCategory,
  );
  getAllCategoryResult((response) => {
    const data = get(response, 'data.getAllCategory', []);
    professionalSkills.value = listToTree(
      data.map((professionalSkill: any) => ({
        key: professionalSkill.code,
        label: professionalSkill.name,
        data: professionalSkill.code,
        parentId: professionalSkill.parentCode,
        children: [],
      })),
      {
        id: 'data',
