    ...
    <form ref='form'>
      <div class='formgrid grid mb-4'>
        ...
        <div class='col-6 field'>
          <label class='font-bold'>{{ t(`hrm.category.effectiveDate`) }}</label>
          <Calendar
            v-model='category.effectiveDate'
            class='w-full'
            icon-display='input'
            name='effectiveDate'
            show-icon/>
        ...
        <div class='col-6 field'>
          <label class='font-bold'>{{ t('hrm.category.allowanceType') }}</label>
          <NamedDropdown
            v-model='category.allowanceType'
            name='allowanceType'
            option-label='label'
            option-value='value'
        ...
        <div class='align-items-center col-6 field flex gap-3'>
          <div class='font-bold'>{{ t(`hrm.category.extend`) }}</div>
          <InputSwitch
            v-model='category.extend'
            name='extend'/>
        </div>
...
import { ALLOWANCE_TYPE } from '@/apps/hrm/constants/category';
...
const setDefaultForm = () => {
  ...
  category.value.extend = true;
  category.value.allowanceType = ALLOWANCE_TYPE.COEFFICIENT;
};

if (props.code) {
  getCategory(props.selectedCategory, props.code).onResult((res) => {
    category.value = cloneDeep(res.data.getCategory);
    category.value.effectiveDate = new Date(category.value.effectiveDate);
  });
} else {
  setDefaultForm();
}
        ...
        if (isCreate) {
          createCategoryMutate({
            ...
            payload: {
              ...omit({
                ...category.value,
                effectiveDate: category.value.effectiveDate
                  ? moment(category.value.effectiveDate).format(DateTimeFormat.ISO_LOCAL_DATE)
                  : null,
              })
            },
          });
          ...
          updateCategoryMutate({
            ...
            payload: removeFields({
              ...category.value,
              effectiveDate: category.value.effectiveDate ? moment(category.value.effectiveDate).format(
...
let allowanceTypes = ref([
  {
    label: t('hrm.category.COEFFICIENT'),
    value: ALLOWANCE_TYPE.COEFFICIENT,
  },
  {
    label: t('hrm.category.PERCENTAGE'),
    value: ALLOWANCE_TYPE.PERCENTAGE,
  },
  {
    label: t('hrm.category.AMOUNT'),
    value: ALLOWANCE_TYPE.AMOUNT,
  },
]);
</script>
