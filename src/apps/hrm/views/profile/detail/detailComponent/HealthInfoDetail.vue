<template>
  <div
    v-if='data'
    class='border-round-xl formgrid gap-2 grid health-info hover:surface-100 mb-1 p-3'>
    <div class='align-items-center col-12 flex justify-content-between'>
      <div class='bg-pink-50 border-round-2xl flex gap-2 px-3 py-2'>
        <span class=''>{{ t('hrm.health.updateDate') }}: </span>
...
const { mutate: deleteEmployeeHealthMutate, onDone: deleteEmployeeHealthDone, } =
    deleteEmployeeHealth();
const confirm = useConfirm();
const confirmDeleteHealthInfo = () => {
  confirm.require({
    ...
    accept: () => {
      handleDeleteHealthInfo();
    },
  });
};

const handleDeleteHealthInfo = () => {
