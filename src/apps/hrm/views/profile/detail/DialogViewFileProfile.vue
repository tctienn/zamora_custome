<template>
  <Dialog
    ...
    :header='`${type == "CENTRAL_ORGANIZATION" ? "Sơ yếu lý lịch (Mẫu 2C/TCTW-98)" : "Sơ yếu lý lịch (Mẫu 2C-BNV/2008)"}`'
    modal
...
import { exportFileEmployee } from '@/apps/hrm/api/rest/employee';
...
function downloadDocx() {
  exportFileEmployee(props.employeeId, props.type)
    .then(file => {
      downloadFile(file!, `${props.employeeId}.docx`);
    });
}
</script>
