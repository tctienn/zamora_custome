...
import { type DocIn, PRIORITIES, type UserDeptRoleInput } from '@/apps/document/model/doc/doc-in';
import { DOC_OUT_STATUSES } from '@/apps/document/model/doc/doc-out';
...
const docOuts = computed(() =>
  resultDocIns.value?.searchAllDocOut?.edges.map((edge: Edge<DocIn>) => edge.node),
);
const total = computed(() => resultDocIns.value?.searchAllDocOut.totalCount || 0);
