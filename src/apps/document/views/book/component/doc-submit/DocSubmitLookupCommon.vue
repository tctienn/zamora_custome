...
import DocSubmitProcessList from '@/apps/document/views/doc/submit/list/item/DocSubmitProcessList.vue';
...
const {
  result: resultDocSubmits,
  loading,
  load,
  refetch,
} = searchAllDocSubmit(search, userDeptRole.value, pageable);
const total = computed(() => resultDocSubmits.value?.searchAllDocSubmit.totalCount);
...
const docSubmits = computed(() =>
  resultDocSubmits.value?.searchAllDocSubmit.edges.map((edge: Edge<DocOut>) => edge.node),
);
...
function goToDetailRoute(router: Router, docSubmit: DocSubmit) {
  router.push({
    ...
    params: { processId: docSubmit.process?.id }
  });
}
