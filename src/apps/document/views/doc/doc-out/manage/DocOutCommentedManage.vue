...
import { docOutCommented } from '@/apps/document/api/graphql/doc-out';
...
const {
  ...
  loading,
  refetch
} = docOutCommented(search, userDeptRole.value, pageable);
const docOuts = computed(() => result.value?.docOutCommented.edges.map((edge: Edge<DocOut>) => edge.node));
const total = computed(() => result.value?.docOutCommented.totalCount);
...
watch(total, (total) => {
  if (total !== undefined && total !== null) {
    updateCount('DocumentDocOutCommentedManage', total);
  }
}, { immediate: true });
onMounted(() => {
  load();
