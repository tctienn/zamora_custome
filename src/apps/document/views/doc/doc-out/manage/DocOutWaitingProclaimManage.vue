...
import { docOutWaitingIssued } from '@/apps/document/api/graphql/doc-out';
...
import DocOutPromulgateList from '@/apps/document/views/doc/doc-out/list/DocOutPromulgateList.vue';
...
const menuActions: DocumentAction[] = [
  DocumentAction.CREATE_PROFILE, DocumentAction.ENTER_BOOK, DocumentAction.PROMULGATE, DocumentAction.PUT_INTO_BOOK
];
...
const docOuts = computed(() =>
  result.value?.docOutWaitingIssued.edges.map(
    (edge: Edge<DocOut>) => edge.node,
  ),
);
const total = computed(() => result.value?.docOutWaitingIssued.totalCount);
...
function goToDetailRoute(router: Router, docOut: DocOut) {
  router.push({ path: `waiting-proclaim/detail/${ docOut?.id }` });
}
...
watch(total, (total) => {
  updateCount('DocumentDocOutWaitingProclaimManage', total);
});
