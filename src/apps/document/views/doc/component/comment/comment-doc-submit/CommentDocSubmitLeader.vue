...
} from '@/apps/document/api/graphql/comment';
import { getProcessByDocSubmitId } from '@/apps/document/api/graphql/submit-process';
...
function handlePublicUsers(): UserDeptRoleInput[] {
  return result.value?.docSubmitRelatedUsers || [];
}
...
const {
  ...
  load: loadUsers,
  variables
} = docSubmitRelatedUsers(props.docId, userDeptRole);
...
} = listCommentsGraphql(ObjectTypeEnum.PHIEU_TRINH, props.docId, isPrivate.value);
const request = computed<PrivateCommentDocRequestInput>(() => {
  return {
    canViewPartner: true,
    objectId: props.docId,
    objectType: ObjectTypeEnum.PHIEU_TRINH,
...
function newChatComment(content: string, attachments: FileList | undefined, selectedUsers: UserDeptRoleInput[]) {
  createChatComment(createFormCommentDocSubmit(content, attachments, selectedUsers, CommentSendType.CHAT, userDeptRole, props.docId, isPrivate.value,))
    .then(value => {
      reloadListComment();
    })
    .catch(toastError);
...
watchEffect(() => {
  getProcessByDocSubmitId(props.docId).onResult(param => {
    commentsFromLog.value = [];

    function mapToComment(inputProcess: SubmitProcess): DocComment & { [key: string]: any } {
      return {
    ...
    const groupByParentId: Record<string, SubmitProcess[]> = groupBy(param.data.getProcessByDocSubmitId, value => value.parentId);
...
if (userDeptRole.roleId == RoleType.LANH_DAO) {
  getUserSupportByUserId(userDeptRole.userId, userDeptRole.deptId).onResult((res) => {
    userSupportLeader.value = res.data?.getUserSupportByUserIdAndDeptId
      .filter((u: UserSupport) => u.supportPermission.includes(SupportPermission.ADVISE_DOC_SUBMIT));
  }
  );
}
