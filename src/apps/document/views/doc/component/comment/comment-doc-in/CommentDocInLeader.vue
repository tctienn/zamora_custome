...
const {
  ...
  load: loadUsers,
  variables,
} = docInRelatedUsers(props.docId, userDeptRole);
watch(
  () => props.docId,
  (docId) => {
    variables.value = {
...
onResult((value) => {
  if (isPrivate.value) {
    privateComments.value = value.data?.listComments;
  } else {
...
watchEffect(() => {
  getAllProcessByDocIdForDrawRealFlow(props.docId, userDeptRole).onResult(
    (param) => {
      commentsFromLog.value = [];

      function mapToComment(
        inputProcess: InComingProcess,
      ): DocComment & { [key: string]: any } {
        return {
...
//For private
if (userDeptRole.roleId == RoleType.LANH_DAO) {
  getUserSupportByUserId(userDeptRole.userId, userDeptRole.deptId).onResult(
    (res) => {
      userSupportLeader.value = res.data?.getUserSupportByUserIdAndDeptId.filter(
        (u: UserSupport) =>
          u.supportPermission.includes(SupportPermission.ADVISE_INCOMING_DOC),
      );
    },
  );
}
