...
import { commentedDocs } from '@/apps/document/api/graphql/doc-in';
...
const {
  ...
  load,
  refetch,
} = commentedDocs(search, userDeptRole.value, pageable);
const docIns = computed(() =>
  resultDocIns.value?.commentedDocs.edges.map((edge: Edge<DocIn>) => edge.node),
);
const total = computed(() => resultDocIns.value?.commentedDocs.totalCount);
...
function goToDetailRoute(router: Router, docIn: DocIn) {
  ...
  // router.push({ path: `commented/detail/${docIn.id}` });
}
...
watch(total, (total) => {
  if (total !== undefined && total !== null) {
    updateCount('DocumentDocInCommentedManage', total);
  }
}, { immediate: true });
