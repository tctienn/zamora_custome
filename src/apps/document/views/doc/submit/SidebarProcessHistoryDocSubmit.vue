      ...
      <TabView
        ...
        <TabPanel
          ...
          <DataTable
            ...
            <column
              header='Người gửi'>
              <template #body='{data: {senderId}}'>
                <AppUser
                  avatar-size='2.5'
                  info-visible
                  :infos='["positionName", "departments"]'
                  name-visible
                  :user-id='senderId'></AppUser>
              </template>
            </column>

            <column
              header='Người nhận'>
              <template #body='{data: {receiverId, isRead}}'>
...
const { onResult: getProcessResult } = getProcessByDocSubmitId(props.docId);
getProcessResult((res) => {
  outgoingProcess.value = res.data.getProcessByDocSubmitId;
});
</script>

<script lang="ts">
export default { name: 'SidebarProcessHistoryDocSubmit' };
</script>
