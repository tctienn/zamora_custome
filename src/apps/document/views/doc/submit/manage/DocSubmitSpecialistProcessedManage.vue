...
import { docSubmitProcessed } from '@/apps/document/api/graphql/doc-submit';
...
const {
  ...
  refetch,
  loading
} = docSubmitProcessed(search, userDeptRole.value, pageable);

const docSubmits = computed(() => result.value?.docSubmitProcessed?.edges.map((edge: Edge<DocSubmit>) => edge.node));
const total = computed(() => result.value?.docSubmitProcessed?.totalCount);
...
watch(total, (total) => {
  updateCount('DocumentSubmitSpecialistProcessedManage', total);
});
onMounted(() => {
  load();
});
