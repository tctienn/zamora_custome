...
function doAction(currProcessId: string, data: DataNextAction) {
  function createFormData(data: DataNextAction) {
    ...
    Object.entries(data.dataNodeMap).forEach(([, value1]) => {
      ...
      value1.processes.forEach((value2, index2) => {
        formData.append(`dataNodes[${index1}].submitProcesses[${index2}].receiverId`, value2.receiverId);
        formData.append(`dataNodes[${index1}].submitProcesses[${index2}].receiverName`, value2.receiverName);
        formData.append(`dataNodes[${index1}].submitProcesses[${index2}].receiverDeptId`, value2.receiverDeptId);
        formData.append(`dataNodes[${index1}].submitProcesses[${index2}].receiverDeptName`, value2.receiverDeptName);
        formData.append(`dataNodes[${index1}].submitProcesses[${index2}].receiverRoleId`, value2.receiverRoleId);
        formData.append(`dataNodes[${index1}].submitProcesses[${index2}].receiverRoleName`, value2.receiverRoleName);
        formData.append(`dataNodes[${index1}].submitProcesses[${index2}].processType`, value2.processType);
      });
      index1++;
    });
  ...
  return submitFlowActionWithFiles(currProcessId, userDeptRole.value, firstEdge.value?.data?.outcome || '', formData);
}
...
async function saveDocAndAction() {
  try {
    ...
    const doc: DocSubmit = await saveDocPromise();
