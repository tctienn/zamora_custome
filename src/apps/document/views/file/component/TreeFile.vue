...
import { deleteFile } from '@/apps/document/api/graphql/file';
...
const tree = computed(() =>
  listToTree(
    props.listFile.map((item: FileInterface) => ({
      key: item.id,
      label: item.fileName,
      data: item,
...
const { mutate: deleteFileMutate, onDone: deleteFileOnDone } = deleteFile();
...
const getNameFile = (id: string) => {
  return props.listFile.find((item: any) => item.id === id)?.fileName;
};
</script>
