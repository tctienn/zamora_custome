<template>
  ...
  <DataTable
    ...
    <Column
      class='text-center'
      field='actionType'
      :header='t("admin.log.actionType")'
      :sortable='true'>
      <template #body='{data: {actionType}}'>
        {{ t(`common.actionAuth.${toLower(actionType)}`) }}
      </template>
    </Column>
    ...
    <Column
      class='text-center'
      field='resultType'
      :header='t("admin.log.resultType")'
      :sortable='true'>
      <template #body='{data: {resultType}}'>
        {{ t(`common.resultAuth.${toLower(resultType)}`) }}
      </template>
    </Column>
...
import { getLogAuthGraphql } from '@/apps/admin/api/graphql/log-auth-graphql-api';
...
} = getLogAuthGraphql(searchParams);

getLogUserResult((result) => {
  const {
    totalCount,
    edges
  } = get(result, 'data.getLogAuth', {
    totalCount: 0,
    edges: [],
  });
