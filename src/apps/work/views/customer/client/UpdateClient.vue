<template>
  <div class='mb-2'>
    <h6 class='margin_small-header text-lg'>{{ t('work.client.updateClient') }}</h6>
  </div>
  <div class='p-fluid'>
    <div class='formgrid grid'>
      <div class='col field'>
        ...
        <InputText
          ...
          autofocus
          :disabled='formClient.id != null'/>
        <InputText
          id='objectCode'
          v-model='formClient.id'
          hidden/>
        <small
...
import { createClient, findClientById, updateClient, } from '@/apps/work/api/graphql/client-tenant-graphql-api';
...
const {
  onResult: findClientByIdOnResult,
  load: findClientByIdLoad
} = findClientById();
let clientData = ref<ClientInterface>({} as ClientInterface);

findClientByIdLoad(undefined, { id: id });
findClientByIdOnResult((res) => {
  clientData.value = res.data.findClientById;
  if (clientData.value) {
    formClient.value.clientCode = clientData.value.clientCode;
    formClient.value.clientName = clientData.value.clientName;
    formClient.value.city = clientData.value.city;
    formClient.value.address = clientData.value.address;
    formClient.value.home = clientData.value.home;
    formClient.value.groupClient = clientData.value.groupClient;
    formClient.value.scale = clientData.value.scale;
    formClient.value.taxCode = clientData.value.taxCode;
    formClient.value.foundingDate = new Date(clientData.value.foundingDate);
    formClient.value.field = clientData.value.field;
    formClient.value.email = clientData.value.email;
    formClient.value.phone = clientData.value.phone;
    formClient.value.note = clientData.value.note;
    formClient.value.id = clientData.value.id;
    formClient.value.contact = clientData.value.contact;
    formClient.value.contactEmail = clientData.value.contactEmail;
    formClient.value.contactPhone = clientData.value.contactPhone;
    formClient.value.position = clientData.value.position;
  }
}
);
...
function fnSaveClient() {
  ...
  if (!isEmpty(formClient.value.email) && !formClient.value.email.match(mailFormatRegex)
      ...
      || !isEmpty(formClient.value.contactEmail) && !formClient.value.contactEmail.match(mailFormatRegex)
      || !isEmpty(formClient.value.contactPhone) && !formClient.value.contactPhone.match(telephoneNumberRegex)) {
    return;
  }
