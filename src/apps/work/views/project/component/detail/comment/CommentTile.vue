<template>
  <div class='border-1 border-round flex flex-column gap-2 p-2 relative surface-border'>
    ...
    <div>
      <div
        v-for='attachment in comment.attachmentPaths'
        :key='attachment'>
        {{ attachment }}
      </div>
      <div
...
const comment = defineModel<ProjectDiscussionInterface>('comment', { required: true });
...
const replyComments = ref<ProjectDiscussionInterface[]>([]);
const canLoadMoreReply = computed(() => totalReplyComment.value > replyComments.value.length);
const deleteComment = inject('deleteComment', (commentId: string) => Promise.resolve());
...
function deleteCommentEvent() {
  confirm({
    ...
    accept: () => {
      deleteComment(comment.value?.id).then(() => {
...
function sentComment(comment: ProjectDiscussionInterface) {
  ...
  replyComments.value.push(comment);
}

function getComment() {
  replyCommentsByRepliedCommentId(comment.value.objectId, comment.value.id, pageable)
    .then(({
      content,
      totalElements
    }) => {
      totalReplyComment.value = totalElements;
      replyComments.value = content || [];
    });
}
  ...
  if (canLoadMoreReply.value) {
    ...
    replyCommentsByRepliedCommentId(comment.value.objectId, comment.value.id, pageable)
      .then(({
        ...
        totalReplyComment.value = totalElements;
        replyComments.value = [...replyComments.value, ...content || []];
      });
  }
}
