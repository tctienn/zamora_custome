  ...
  <DataTable
    ...
    <template #groupheader='{data}'>
      <span
        class='font-bold'
        style='color: #0F6CBD'>
        {{ taskGroupMap[data?.groupId]?.node?.name || 'Không xác định' }}
      </span>
    </template>
  </DataTable>
...
import { searchByTaskGroup } from '@/apps/work/api/graphql/task-group';
...
const { result: resultTaskGroup } = searchByTaskGroup({
  pageable: {
    page: 0,
    size: 99999999,
  }
});
const taskGroupMap = computed<Record<string, { id: string; node: Record<string, any> }>>(() => {
  const taskGroupData = resultTaskGroup.value?.searchByTaskGroup?.edges || [];
  return associateBy(taskGroupData, item => item?.node?.id);
});
...
onMounted(() => {
  ...
  props.tasks.forEach(e => e?.groupId && groupIds.add(e?.groupId));
  //Null vì có công việc không nằm trong nhóm nào
  expandedRowGroups.value = [...groupIds, null];
});
</script>
