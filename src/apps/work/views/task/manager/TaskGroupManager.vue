<template>
  <Toolbar class='align-content-center h-3rem mb-2 px-2 py-0 surface-0'>
    <template #start>
      <div class='align-items-center flex gap-2'>
        <div class='font-bold'>
          {{ t(`media.common.list`) }} {{ t(`work.task.taskGroup`) }}
        </div>
      </div>
    </template>
    <template #end>
  ...
  <div class='h-full surface-0'>
    <DataTable
      ...
      <Column
        field='name'
        :header='t(`work.task.taskGroup`)'></Column>
...
import { deleteTaskGroup, searchByTaskGroup, } from '@/apps/work/api/graphql/task-group';
import type { Edge } from '@/apps/work/model/project';
import FormWorkGroup from '@/apps/work/views/task/component/FormTaskGroup.vue';
...
const {
  ...
  searchByTaskGroup(searchParams);
const {
  mutate: deleteMutate,
  onDone: deleteOnDone
} = deleteTaskGroup();
...
getResult((res) => {
  const {
    totalCount,
    edges
  } = get(res, 'data.searchByTaskGroup', {
    totalCount: 0,
    edges: [],
  });
...
const confirmDelete = () => {
  confirm.require({
    message: t('common.confirmDelete', {
      itemType: t('work.task.taskGroup'),
