      ...
      <template #expansion='orgSlot'>
        <DataTable
          ...
          <Column
            ...
            <template #body='{ data: { participantName }, index }'>
...
import { findAllTaskOfUsers } from '@/apps/work/api/graphql/report';
...
const {
  onResult: findAllTaskOfUsersResult,
  refetch: findAllTaskOfUsersRefetch
} =
  findAllTaskOfUsers({ pageable });

const groupedByOrganization = computed(() =>
  groupData(data.value, organizations.value)
);

findAllTaskOfUsersResult((res) => {
  const { edges } = get(res, 'data.findAllTaskOfUsers', { edges: [], });
  if (isLoadMore.value) {
    data.value = [
      ...data.value,
...
watch(groupedByOrganization, (newVal) => {
  ...
  expandedParticipants.value = newVal.reduce((acc: any, org: any) => {
    org.tasks.forEach((participant: any) => {
      acc[participant.participantName] = true;
    });
    return acc;
  }, {});
});
  ...
  organizations?.forEach((org: any) => {
    ...
    const groupedParticipants = Object.values(
      orgTasks.reduce((acc: any, task: any) => {
        const name = task.participantName;
        if (!acc[name]) {
          acc[name] = {
