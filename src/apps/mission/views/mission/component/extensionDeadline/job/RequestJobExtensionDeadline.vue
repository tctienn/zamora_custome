<template>
  <Dialog
    ...
    :header="t('Gia hạn thời gian thực hiện công việc')"
    :style='{
      width: "60vw",
    }'
...
import { saveJobDeadlineExtensionRequest } from '@/apps/mission/api/graphql/job-api';
  ...
  v$.value.$validate()
    .then((isValid) => {
      if (isValid && deadlineExtension.value) {
        ...
        mutate({ jobDeadlineExtensionInput: payload, });
      }
    });
};
