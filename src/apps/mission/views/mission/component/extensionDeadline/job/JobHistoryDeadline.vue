<template>
  <Dialog
    ...
    :header="t('Lịch sử gia hạn công việc')"
    style='width: 90vw;'
    @hide='handleClose'>
    <DataTable
      :loading = 'isLoading'
...
import { getAllJobDeadlineExtensionRequest } from '@/apps/mission/api/graphql/job-api';
...
const {
  onResult: getJobHistoryDeadlineResult,
  onError: getJobHistoryDeadlineError
} =
  getAllJobDeadlineExtensionRequest(props.id || '');

getJobHistoryDeadlineResult((res) => {
  const value = get(res, 'data.getAllJobDeadlineExtensionRequest');
