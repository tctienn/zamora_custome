<template>
  <Dialog
    ...
    :header="t('Lịch sử gia hạn hoạt động')"
    style='width: 90vw;'
    @hide='handleClose'>
    <DataTable
      :loading = 'isLoading'
...
import { getAllActivityDeadlineExtensionRequest } from '@/apps/mission/api/graphql/activity-api';
...
const {
  onResult: getActivityHistoryDeadlineResult,
  onError: getActivityHistoryDeadlineError
} =
  getAllActivityDeadlineExtensionRequest(props.id || '');

getActivityHistoryDeadlineResult((res) => {
  const value = get(res, 'data.getAllActivityDeadlineExtensionRequest');
...
getActivityHistoryDeadlineError(()=>{
  isLoading.value = false;
});
