<template>
  <Dialog
    ...
    :header="t('Duyệt gia hạn thời gian thực hiện hoạt động')"
    :style='{
      width: "60vw",
    }'
...
import { generalLeaderApproveActivityDeadlineExtensionRequest, getDetailActivityDeadlineExtensionRequest } from '@/apps/mission/api/graphql/activity-api';
...
if (props.id) {
  const { onResult } = getDetailActivityDeadlineExtensionRequest(props.id);
  onResult((res) => {
    const value = get(res, 'data.getDetailActivityDeadlineExtensionRequest');
    console.log(value);
