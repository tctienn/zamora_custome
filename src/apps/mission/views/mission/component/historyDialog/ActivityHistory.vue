...
import { getActivityHistoryById } from '@/apps/mission/api/graphql/activity-api';
import type { IMissionHistory } from '@/apps/mission/model/mission/mission';
import { DEFAULT_PAGE_SIZE } from '@/common/constants';

const activityHistory = ref<IMissionHistory[]>([]);
...
const {
  onResult: getActivityHistoryResult,
  onError: getActivityHistoryError
} =
  getActivityHistoryById(searchParams);

getActivityHistoryResult((res) => {
  const {
    totalRecords,
    content
  } = get(res, 'data.getActivityHistoryById', {
    totalRecords: 0,
    content: [],
  });
