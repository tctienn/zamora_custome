    ...
    <div
      ...
      <div
        ...
        <div class='flex flex-column min-h-full row-gap-3'>
          <template
            v-for='(mission, index) in column.listMission'
            :key='index'>
            <div
              draggable='true'
              @contextmenu='toggleMenu($event, mission)'
              @drag='dragWork'
              @dragstart='dragWorkStart(mission, $event)'>
              <slot
                :mission='mission'
                name='kanbanItem'>
              </slot>
...
import { type MissionInterface, StatusMission } from '@/apps/mission/model/mission/mission';
...
const columns = computed(() =>
  board.value.columns.map(c => ({
    ...c,
    listMission: props.missions.filter(m => m.node.status === c.type)
  }))
);

console.log(columns.value);

function dragWorkStart(mission: MissionInterface, event: DragEvent) {
...
function toggleMenu(e: Event, mission: MissionInterface) {
