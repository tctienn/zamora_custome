  ...
  <DataTable
    ...
    <template #groupheader='{data}'>
      <span
        ...
        style='color: #0F6CBD; margin-bottom: 20px;'>{{ data.missionName }}</span>
    </template>
    <template #empty>
    ...
    <Column
      ...
      <template #body='{ data }'>
        <div
          ...
          <p class='m-0'>{{ data.activityName }}</p>
...
watch(() => props.activityData, (newVal) => {
  if (newVal?.length) {
    const uniqueCampaignIds = [...new Set(newVal.map(item => item.missionId))];
